...
public class Grid<T> {
	...
	private static final Logger LOG = AuroraLogs.getLogger(Grid.class);
	public static <T> Boolean func_check_null(int[] p, T t) { return true; }
	public static <T> Boolean func_check_occupied(int[] p, T t) { return t != null; }
	public static <T> Boolean func_check_unoccupied(int[] p, T t) { return t == null; }
	public static <T> T func_make_null(int[] p, T t) { return null; }

	private final int[] position, offset, maximum;
	private final Object[] grid;
	/**
	 * Creates a new single element grid with the specified dimension. This is
	 * for testing only.
	 *
	 * @param dim
	 */
	...
	/**
	 * Creates a grid with the specified central position and maximum bounds.
	 * Note the actual size is 2*max+1 for each dimension so that there is a
	 * central focus.
	 *
	 * @param pos
	 * @param max
	...
	private int getRef(int[] p) {
		...
		for (int i = 0; i < position.length; i++) {
			// TODO: Verify
			e = e * (2 * maximum[i] + 1) + (maximum[i] + position[i] - p[i] + offset[i]) % (2 * maximum[i] + 1);
		}
		return e;
	}
	...
	public T get(int[] p) {
		...
		if (ref >= grid.length) {
			LOG.log(Level.WARNING, "Attempted to take an element from beyond the grid range! There is a problem here!");
			return null;
		}
	 ...
	 */
	public boolean set(int[] p, BiFunction<int[], T, Boolean> check, BiFunction<int[], T, T> make) {
		int ref = getRef(p);
		if (ref < 0) {
			return false;
		}
