...
public final class LDRef {
	 ...
	 */
	public LDRef(LDAng ang) {
		set(ang);
	}

	/**
	 * Creates a new reference frame, such that the three provided vectors form
	 * the X, Y, Z basis for the space. The AB direction is the z-axis, the
	 * cross product of AB and AC form the y-axis, and the cross product of the
	 * y-axis and z-axis form the x-axis.
	 *
	 * @param vecA Position Vector A
	 * @param vecB Position Vector B
	 * @param vecC Position Vector C
	 */
	public LDRef(LDVec vecA, LDVec vecB, LDVec vecC) {
		set(vecA, vecB, vecC);
	}

	/**
	 * Creates a new reference frame depending on the number of vectors
	 * provided. If one is provided, the new reference frame is translated by
	 * the given vector. If three are provided, the vectors form the X, Y, Z
	 * basis for the space. Otherwise, an <code>IllegalArgumentException</code>
	 * is thrown.
	 *
	 * @param vecs The vectors
	 */
	public LDRef(LDVec[] vecs) {
		set(vecs);
	}

	/**
	 * Creates a new reference frame with the provided column-major 4x4 matrix
	 * given as a 16 float array. The matrix given should be a special
	 * orthogonal affine matrix.
	 *
	 * @param matrix The matrix.
	 */
	public LDRef(float[] matrix) {
		set(matrix);
	}
	 ...
	 * provided, then returns this. If one is provided, the new reference frame
	 * is translated by the given vector. If three are provided, the vectors
	 * form the X, Y, Z basis for the space. Otherwise, an
	...
	/**
	 * Translates the reference frame globally, that is before the
	 * transformation described by this frame, then returns this. It is
	 * analogous to moving an object outside of this reference frame.
	 *
	 * @param vec The translation vector
	 * @return This
	 */
	 ...
	 * @return This
	 */
	public LDRef multiplyLocally(LDRef ref) {
		return mult(ref, this, this);
	}

	/**
	 * Transforms this reference frame globally with the provided reference
	 * frame transformation, that is before the transformation described by this
	 * frame, then returns this. It is analogous to moving an object outside of
	 * this reference frame.
	 *
	 * @param ref The transformation frame
	 * @return This
	 */
	public LDRef multiplyGlobally(LDRef ref) {
	...
	/**
	 * Returns the vector transformed to inside this reference frame. The
	 * position flag should be true if the vector is a position vector.
	 *
