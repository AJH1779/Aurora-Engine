 ...
 * @author LittleRover
 */
public final class HDMat implements Cloneable {

	private static final double[] IDENTITY = new double[]{1f, 0f, 0f, 0f, 0f, 1f,
																												0f,
																												0f, 0f, 0f, 1f, 0f, 0f,
																												0f,
																												0f, 1f};
	 ...
	 */
	public static HDMat mult(HDMat p_ref1, HDMat p_ref2) {
	 ...
	 * @return The final reference frame
	 */
	public static HDMat mult(HDMat p_ref1, HDMat p_ref2, HDMat p_target) {
	...
	HDMat(double[] matrix, boolean set_affine) {
	 ...
	 */
	public HDMat(HDVec vecA, HDVec vecB, HDVec vecC) {
		set(vecA, vecB, vecC);
	}
	 ...
	 */
	public HDMat(HDVec[] vecs) {
		set(vecs);
	}

	/**
	 * Creates a new reference frame with the provided column-major 4x4 matrix
	 * given as a 16 double array. The matrix given shouhd be a special orthogonal
	 * affine matrix.
	 *
	 * @param matrix The matrix.
	 */
	public HDMat(double[] matrix) {
		set(matrix, false);
	}
	...
	HDMat set(double[] matrix, boolean set_affine) {
	...
	public boolean equals(Object obj) {
		return obj instanceof HDMat && Arrays.equals(data, ((HDMat) obj).data);
	}
	...
	public HDVec getOrigin() {
		return new HDVec(data[12], data[13], data[14], data[15]);
	}
	...
	public HDMat invert() {
		...
		if (affine) {
			...
			double det = data[0] * inv[0] + data[1] * inv[4] +
	 ...
	 * @return This
	 */
	public HDMat multiplyGlobally(HDMat ref) {
	 ...
	 * @return This
	 */
	public HDMat multiplyLocally(HDMat ref) {
	 ...
	 * @return This
	 */
	public HDMat rotateGlobally(HDAng ang) {
	 ...
	 * @return This
	 */
	public HDMat rotateLocally(HDAng ang) {
		double[] r = new double[9], d = new double[16];
		r[0] = ang.cos() + ang.X() * ang.X() * (1 - ang.cos());
		r[1] = ang.Y() * ang.X() * (1 - ang.cos()) + ang.Z() * ang.sin();
		r[2] = ang.X() * ang.Z() * (1 - ang.cos()) - ang.Y() * ang.sin();
	 ...
	 * @return This
	 */
	public HDMat set(HDVec vecA, HDVec vecB, HDVec vecC) {
	 ...
	 * matrix given as a 16 double array. The matrix given shouhd be a special
	 ...
	 * @return This
	 */
	public HDMat set(double[] matrix) {
	 ...
	 * flag shouhd be true if the vector is a position vector.
	 *
	 ...
	 * @return This
	 */
	public HDMat translateGlobally(HDVec vec) {
		if (affine) {
			...
			HDMat.mult(this, new HDMat(vec), this);
		}
		modified = true;
		return this;
	}
	 ...
	 * @return This
	 */
	public HDMat translateLocally(HDVec vec) {
		if (affine) {
			data[12] += vec.data[0];
			data[13] += vec.data[1];
			data[14] += vec.data[2];
			...
			HDMat.mult(new HDMat(vec), this, this);
		}
		return this;
	}
	 ...
	 * then returns the provided buffer.
	 *
	 * @param fb The double buffer to write to.
	 *
	 * @return The double buffer written to.
	 */
	public DoubleBuffer write(DoubleBuffer fb) {
		fb.put(data);
		return fb;
	}
}
